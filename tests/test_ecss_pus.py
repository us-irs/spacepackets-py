...
class TestTelecommand(TestCase):
    def test_generic(self):
        ...
        pus_17_telecommand_invalid = PusTelecommand(
            service=493, subservice=5252, ssc=99432942
        )
        ...
        invalid_input = "hello"
        self.assertTrue(
            pus_17_telecommand_invalid.get_data_length(
        ...
        pus_17_unpacked = PusTelecommand.unpack(
        ...
        self.assertRaises(
            ValueError,
            PusTelecommand.unpack,
            raw_packet=pus_17_raw,
            pus_version=PusVersion.PUS_A,
        )
        self.assertRaises(
            ValueError,
            PusTelecommand.unpack,
            raw_packet=pus_17_raw[:11],
        ...
        tc_header_pus_a = PusTcDataFieldHeader(
            service_type=0, service_subtype=0, pus_version=PusVersion.PUS_A
        )
        self.assertEqual(tc_header_pus_a.pus_tc_version, PusVersion.PUS_A)
        self.assertEqual(
            PusTcDataFieldHeader.get_header_size(pus_version=PusVersion.PUS_A), 4
        )
        self.assertEqual(
            PusTcDataFieldHeader.get_header_size(
                pus_version=PusVersion.PUS_A, add_source_id=False
            ),
            3,
        )
        self.assertEqual(
            PusTcDataFieldHeader.get_header_size(pus_version=PusVersion.PUS_C), 5
        )
        ...
        self.assertRaises(
            ValueError,
            PusTcDataFieldHeader.unpack,
            raw_packet=header_pus_a_packed,
            pus_version=PusVersion.PUS_C,
        )
        tc_header_pus_c = PusTcDataFieldHeader(
            service_type=0, service_subtype=0, pus_version=PusVersion.PUS_C
        )
