...
class TestTmFrame(TestCase):
    ...
    def test_unpack_transfer_frame_invalid_fefc(self):
        packed = b'\x00?\x00\x00\x1f\xfe\x00\x00\x01\x02\x01\x00\x00\x00\x01\x23'  # actual is \x81\x4a
        self.assertRaises(InvalidCrcCcitt16, TmTransferFrame.unpack, packed, 16, True)
    ...
    def test_pack_unpack_transfer_frame_sec_header_ocf_fefc(self):
        ...
        dfs = TransferFrameDataFieldStatus(secondary_header_flag=True, sync_flag=False, packet_order_flag=False, segment_len_id=3, first_header_pointer=2046)
        primary_header = TmFramePrimaryHeader(master_channel_id=mci, vc_id=7, ocf_flag=True, master_ch_frame_count=0, vc_frame_count=0, frame_datafield_status=dfs)
        secondary_header = TransferFrameSecondaryHeader(0, 1, b"\x05")
        tm_frame = TmTransferFrame(17, primary_header, secondary_header, b"\x00\x01\x02", b'\x10\x00\x00\x00', b'\x62\x6a')
    ...
    def test_pack_unpack_transfer_frame_sec_header_no_ocf_fefc(self):
        mci = MasterChannelId(transfer_frame_version=0, spacecraft_id=3)
        dfs = TransferFrameDataFieldStatus(secondary_header_flag=True, sync_flag=False, packet_order_flag=False, segment_len_id=3, first_header_pointer=2046)
        primary_header = TmFramePrimaryHeader(master_channel_id=mci, vc_id=7, ocf_flag=False, master_ch_frame_count=0, vc_frame_count=0, frame_datafield_status=dfs)
        secondary_header = TransferFrameSecondaryHeader(0, 1, b"\x05")
        tm_frame = TmTransferFrame(13, primary_header, secondary_header, b"\x00\x01\x02", None, b'\x42\x73')
        packed = tm_frame.pack(tm_frame)
    ...
    def test_pack_unpack_transfer_frame_sec_header_ocf_no_fefc(self):
        ...
        primary_header = TmFramePrimaryHeader(master_channel_id=mci, vc_id=7, ocf_flag=True, master_ch_frame_count=0, vc_frame_count=0, frame_datafield_status=dfs)
        secondary_header = TransferFrameSecondaryHeader(0, 1, b"\x05")
        tm_frame = TmTransferFrame(15, primary_header, secondary_header, b"\x00\x01\x02", b'\x10\x00\x00\x00', None)
        packed = tm_frame.pack(tm_frame)
    ...
    def test_pack_unpack_transfer_frame_sec_header_no_ocf_no_fefc(self):
        ...
        primary_header = TmFramePrimaryHeader(master_channel_id=mci, vc_id=7, ocf_flag=False, master_ch_frame_count=0, vc_frame_count=0, frame_datafield_status=dfs)
        secondary_header = TransferFrameSecondaryHeader(0, 1, b"\x05")
        tm_frame = TmTransferFrame(11, primary_header, secondary_header, b"\x00\x01\x02", None, None)
        packed = tm_frame.pack(tm_frame)
    ...
    def test_pack_unpack_transfer_frame_no_sec_header_ocf_fefc(self):
        mci = MasterChannelId(transfer_frame_version=0, spacecraft_id=3)
        dfs = TransferFrameDataFieldStatus(secondary_header_flag=False, sync_flag=False, packet_order_flag=False, segment_len_id=3, first_header_pointer=2046)
        primary_header = TmFramePrimaryHeader(master_channel_id=mci, vc_id=7, ocf_flag=True, master_ch_frame_count=0, vc_frame_count=0, frame_datafield_status=dfs)
        tm_frame = TmTransferFrame(15, primary_header, None, b"\x00\x01\x02", b'\x10\x00\x00\x00', b'\x60\xad')
        packed = tm_frame.pack(tm_frame)
    ...
    def test_pack_unpack_transfer_frame_no_sec_header_ocf_no_fefc(self):
        ...
        dfs = TransferFrameDataFieldStatus(secondary_header_flag=False, sync_flag=False, packet_order_flag=False, segment_len_id=3, first_header_pointer=2046)
        primary_header = TmFramePrimaryHeader(master_channel_id=mci, vc_id=7, ocf_flag=True, master_ch_frame_count=0, vc_frame_count=0, frame_datafield_status=dfs)
        tm_frame = TmTransferFrame(13, primary_header, None, b"\x00\x01\x02", b'\x10\x00\x00\x00', None)
        packed = tm_frame.pack(tm_frame)
