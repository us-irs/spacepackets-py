...
class Service1TmTest(TestCase):
    def setUp(self) -> None:
        self.def_apid = 0x02
        ping_tc = PusTc(apid=self.def_apid, service=17, message_subtype=1)
    ...
    def test_basic(self):
        ...
        self.assertEqual(self.srv1_tm.message_subtype, 3)
    ...
    def test_legacy_subservice_ctor_and_alias(self):
        with warnings.catch_warnings(record=True) as caught:
            ...
            srv1_tm = Service1Tm(
                apid=self.def_apid,
                subservice=MessageSubtype.TM_START_SUCCESS,
                timestamp=TEST_STAMP,
            )
            self.assertEqual(srv1_tm.subservice, MessageSubtype.TM_START_SUCCESS)
        self.assertGreaterEqual(len(caught), 1)
        self.assertIs(Subservice, MessageSubtype)
    ...
    def test_service_1_tm_acc_success(self):
        self._generic_test_srv_1_success(MessageSubtype.TM_ACCEPTANCE_SUCCESS)

    def test_service_1_tm_start_success(self):
        self._generic_test_srv_1_success(MessageSubtype.TM_START_SUCCESS)

    def test_service_1_tm_step_success(self):
        self._generic_test_srv_1_success(MessageSubtype.TM_STEP_SUCCESS)

    def test_service_1_tm_completion_success(self):
        self._generic_test_srv_1_success(MessageSubtype.TM_COMPLETION_SUCCESS)

    def _generic_test_srv_1_success(self, message_subtype: MessageSubtype):
        pus_tc = PusTc(apid=self.def_apid, service=17, message_subtype=1)
        helper_created = None
        step_id = None
        if message_subtype == MessageSubtype.TM_ACCEPTANCE_SUCCESS:
            helper_created = create_acceptance_success_tm(
                apid=self.def_apid, pus_tc=pus_tc, timestamp=TEST_STAMP
            )
        elif message_subtype == MessageSubtype.TM_START_SUCCESS:
            helper_created = create_start_success_tm(self.def_apid, pus_tc, TEST_STAMP)
        elif message_subtype == MessageSubtype.TM_STEP_SUCCESS:
            step_id = PacketFieldEnum.with_byte_size(1, 4)
            helper_created = create_step_success_tm(self.def_apid, pus_tc, step_id, TEST_STAMP)
        elif message_subtype == MessageSubtype.TM_COMPLETION_SUCCESS:
        ...
        if helper_created is not None:
            self._test_srv_1_success_tm(pus_tc, helper_created, message_subtype, step_id)

    def test_service_1_tm_acceptance_failure(self):
        self._generic_test_srv_1_failure(MessageSubtype.TM_ACCEPTANCE_FAILURE)

    def test_service_1_tm_start_failure(self):
        self._generic_test_srv_1_failure(MessageSubtype.TM_START_FAILURE)

    def test_service_1_tm_step_failure(self):
        self._generic_test_srv_1_failure(MessageSubtype.TM_STEP_FAILURE)

    def test_service_1_tm_completion_failure(self):
        self._generic_test_srv_1_failure(MessageSubtype.TM_COMPLETION_FAILURE)
    ...
    def _test_srv_1_success_tm(
        ...
        self.assertEqual(srv_1_tm.pus_tm.message_subtype, message_subtype)
        ...
        if step_id is not None and message_subtype == MessageSubtype.TM_STEP_SUCCESS:
            self.assertEqual(srv_1_tm_unpacked.step_id, step_id)

    def _generic_test_srv_1_failure(self, message_subtype: MessageSubtype):
        pus_tc = PusTc(apid=self.def_apid, service=17, message_subtype=1)
        ...
        helper_created = None
        step_id = None
        if message_subtype == MessageSubtype.TM_ACCEPTANCE_FAILURE:
            helper_created = create_acceptance_failure_tm(
                self.def_apid, pus_tc, failure_notice, TEST_STAMP
            )
        elif message_subtype == MessageSubtype.TM_START_FAILURE:
            helper_created = create_start_failure_tm(
                self.def_apid, pus_tc, failure_notice, TEST_STAMP
            )
        elif message_subtype == MessageSubtype.TM_STEP_FAILURE:
        ...
        elif message_subtype == MessageSubtype.TM_COMPLETION_FAILURE:
