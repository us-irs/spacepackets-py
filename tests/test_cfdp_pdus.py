...
from spacepackets.cfdp.tlv import CfdpTlv, TlvTypes, FileStoreResponseTlv, FilestoreActionCode, \
    FilestoreResponseStatusCode, EntityIdTlv, FaultHandlerOverrideTlv, FileStoreRequestTlv, \
...
class TestPdus(TestCase):
    ...
    def test_metadata_pdu(self):
        ...
        self.assertEqual(
            pdu_with_two_options.packet_len,
            expected_len
        )
        self.assertEqual(
            len(pdu_with_two_options_raw),
            expected_len
        )
        pdu_with_two_options.source_file_name = 'hello.txt'
        expected_len = header_len + 5 + 1 + 10 + option_0.packet_len + option_1.packet_len
        self.assertEqual(pdu_with_two_options.packet_len, expected_len)
        pdu_with_two_options.dest_file_name = 'hello2.txt'
        expected_len = header_len + 5 + 11 + 10 + option_0.packet_len + option_1.packet_len
        self.assertEqual(pdu_with_two_options.packet_len, expected_len)
